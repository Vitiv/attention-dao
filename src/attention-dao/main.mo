...
actor Main {
            ...
            switch (ledger) {
              case (?l) {
                ...
                if (transferResult) {
                  ...
                  members.put(transfer.recipient, { id = transfer.recipient; votingPower = newBalance });
                  #ok();
                } else {
                  #err("Transfer failed");
                };
      ...
      case (#transferFunds(transfer)) {
        switch (ledger) {
          case (?l) {
            ...
            switch (result) {
              case (true) {
                members.put(transfer.recipient, { id = transfer.recipient; votingPower = await l.getBalance(transfer.recipient) });
                #ok;
              };
              case (false) {
  ...
  public shared (msg) func removeMember(id : Principal) : async Result.Result<(), Text> {
    ...
    switch (members.remove(id)) {
      ...
      case (?_) #ok(); // TODO clear member balance
    };
  };
