...
module {
  ...
  public func runTests(testActor : TestActorInterface) : async () {
    ...
    let testPrincipal2 = Principal.fromText("mmt3g-qiaaa-aaaal-qi6ra-cai");
  ...
  private func testCreateProposal(testActor : TestActorInterface, p : Principal) : async Nat {
    Debug.print("Test: Creating proposal");
    ...
    switch (result) {
      case (#ok(id)) {
        Debug.print("Test passed: Proposal created successfully with ID: " # debug_show (id));
      ...
      case (#err(e)) {
        Debug.print("Test failed: Could not create proposal. Error: " # debug_show (e));
        assert (false);
  ...
  private func testVoteYes(testActor : TestActorInterface, proposalId : Nat, p2 : Principal, p1 : Principal) : async () {
    Debug.print("Test 6-yes: Voting on proposal");
    func calculateVotingPower(votes : [(Principal, DAO.Vote)], predicate : (DAO.Vote) -> Bool) : Nat {
      let r = Array.foldLeft<(Principal, DAO.Vote), Nat>(
        ...
        0,
        func(acc : Nat, vote : (Principal, DAO.Vote)) : Nat {
          acc + vote.1.votingPower;
        },
      );
    };
    ...
    Debug.print("testVoteYes creating proposal 1 result: " # debug_show (res));
    ...
    switch (proposal) {
      ...
      case (null) {
        Debug.print("Test 6-yes failed: Could not retrieve proposal after voting");
        assert (false);
      };
    };
  };

  private func testVoteNo(testActor : TestActorInterface, proposalId : Nat, p1 : Principal, p2 : Principal) : async () {
    Debug.print("Test 6-no: Voting on proposal");
    func calculateVotingPower(votes : [(Principal, DAO.Vote)], predicate : (DAO.Vote) -> Bool) : Nat {
      let r = Array.foldLeft<(Principal, DAO.Vote), Nat>(
        Array.filter<(Principal, DAO.Vote)>(votes, func(vote : (Principal, DAO.Vote)) : Bool { predicate(vote.1) }),
        0,
    ...
    switch (voteResult1) {
      case (#ok) {
        Debug.print("First vote successful");
        // Creating voting proposals
        let res = await testActor.createVotingRewardsProposal(p1, amount);
        Debug.print("testVoteNo creating proposal 1 result: " # debug_show (res));
      };
      case (#err(e)) Debug.print("First vote failed: " # debug_show (e));
    };
    ...
    switch (voteResult2) {
      case (#ok) {
        Debug.print("Second vote successful");
        let res1 = await testActor.createVotingRewardsProposal(p2, amount);
        Debug.print("testVoteNo creating proposal 2 result: " # debug_show (res1));
      };
      case (#err(e)) Debug.print("Second vote failed: " # debug_show (e));
    };
    ...
    switch (proposal) {
      ...
      case (null) {
        Debug.print("Test 6-no failed: Could not retrieve proposal after voting");
        assert (false);
      };
    };
  };
